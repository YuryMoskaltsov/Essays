...
russia_gini = WorldBankData("Gini Index","Russian Federation", df_gini, 2003, 2015)
...
russia_gdp = WorldBankData("GDP Per Capita","Russian Federation", df_gdp, 2003, 2013)
...
negative_dem_score = [-elem for elem in russia_dem_score.normalizeY() ]
balanced_putin_support = [(elem-0.7) for elem in putin_support.normalizeY()]
simulation = []
for i in range(2003,2006):
    simulation.append(((i-2002))*-0.025)
for i in range(2006,2008):
    simulation.append(-(((-i+2002)*6**2)-0.5)/200)
for i in range(2008,2014):
    simulation.append(((i-2002))**2/100)
    
    
print(regr_putin_support_v_gdp.graph_compare("GDP v Putin Support v Democracy Index", "Years", russia_gdp.getX(), ["GDP", "Putin Support", "Democracy Index", "Simulation"], [russia_gdp.normalizeY(), balanced_putin_support, negative_dem_score, simulation]))
...
#russia_gdp.graph_line("GDP", "USD Thousands")
